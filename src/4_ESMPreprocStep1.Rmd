---
title: "ESM Preprocessing (Revol et al. 2024) Steps 1 - 5"
author: "wxder"
date: "`r Sys.Date()`"
output: 
  html_document:
...
`r txt('esm-mod','Modification',"data are subsetted to include only participants who completed Part2 (3 days of EMA data collection).")` 
```{r}
...
```{r}
# Define missing values for EMA variables to be used study analyses
...
`r txt('esm-mod','Modification',"preparing variables for numeric formatting by addressing issues leading to variables being formatted as character. The variable smoked_since_last_survey gave participants the option to select "6+" which, while useful for other analyses, prevents us from formatting the variable as numeric. The chunk below convert's "6+" to 6 for the variable smoked_since_last_survey.")`

`r txt('esm-mod','Modification',"the below chunk also addresses an issue regarding data capturing instances where participants 'opted out'. In order to maintain these data but in a format compatible with numeric formatting, instances of 'opted out' are recoded to 999.")` 

`r txt('esm-mod','Modification',"the below chunk also formats numeric variables as numeric.")`
```{r}
part2beeped <- part2beeped %>%
  filter(as.numeric(participant_id) %in% subset_ids) %>%
  mutate(
    smoked_since_last_survey = case_when(
      smoked_since_last_survey == "opted out" ~ 999,
      smoked_since_last_survey == "6+" ~ 6,
      TRUE ~ as.numeric(smoked_since_last_survey)
    ),
    mood = as.numeric(ifelse(mood == "opted out", 999, mood)),
    smoke = as.numeric(ifelse(smoke == "opted out", 999, smoke)),
    before_survey = as.numeric(ifelse(before_survey %in% c("opted out", ""), 999, before_survey)),
    unpleasant = as.numeric(ifelse(unpleasant == "opted out", 999, unpleasant)),
    pleasant = as.numeric(ifelse(pleasant == "opted out", 999, pleasant)),
    prior_mood = as.numeric(ifelse(prior_mood == "opted out", 999, prior_mood)),
    participant_id = as.numeric(ifelse(participant_id == "opted out", 999, participant_id)),
    session_id = as.numeric(ifelse(session_id == "opted out", 999, session_id))
  )
```
#Stopped here thursday 2.11!

`r txt('esm-mod','Modification',"run the 'clean_names' function in the janitor package, so that variable names with hyphens become variable names with underscores and all text in variable names becomes lowercase.")`
```{r}
