...
**Participant distribution plot:** displays a density curse for each participant, regrouping per variable ('pos_aff' and 'neg_aff'). 
```{r}
study_sample %>%
  gather(key = var, value = smoke, unpleasant, pleasant) %>%
  ggplot(aes (x = value, color=factor(participant_id))) +
      geom_density(alpha = 0.5) +
      facet_grid(var~.) +
        labs(title="Participants' distributions of unpleasant and pleasant", x="value", color="id")
```


**Correlation plot** of variables 'pos_aff', 'neg_aff' and 'perc_fun_signaled'.
It displays the correlation scores (along with significance test result), the distribution and the scatterplot between two variables. 
```{r}
study_sample %>%
  select(participant_id, smoke, pleasant, unpleasant) %>%
  pivot_longer(
    cols = c(smoke, pleasant, unpleasant),
    names_to = "var",
    values_to = "value"
  ) %>%
  ...
  ggplot(aes(x = value, color = factor(participant_id))) +
    ...
    facet_grid(var ~ ., scales = "free") +
    labs(
      title = "Participants' distributions of smoke, pleasant, and unpleasant",
      x = "Value",
...
**Time series** for the **'pos_aff'** variable of each dyads' partners.
```{r, fig.height=8, fig.width=8}
ggplot(data = data, aes(x = obsno, y = pos_aff, color=factor(role))) +
  geom_line() +
  facet_wrap(dyad~., ncol=3) +
        labs(title="Times series of pos_aff", x="Observation number", color="role")
```

**Time series** for the **'neg_aff'** variable of each dyads' partners.
```{r, fig.height=8, fig.width=8}
ggplot(data = data, aes(x = obsno, y = neg_aff, color=factor(role))) +
  geom_line() +
  facet_wrap(dyad~., ncol=3) +
        labs(title="Times series of neg_aff", x="Observation number", color="role")
```


**Contemporaneous dyad plot**: the plot is composed of scatterplots for the 'pos_aff' variables, with the father's pos_aff on the x-axis and the mother's pos_aff on the y-axis.
It helps investigating the contemporaneous relationship between the father and the mother within dyads.
```{r, fig.height=8, fig.width=8}
ggplot(data = data_dyad, aes(x = pos_aff_2, y = pos_aff_1)) +
  geom_jitter() +
  facet_wrap(dyad~., ncol=3) +
  labs(title="Contemporaneous dyad relationship in pos_aff", x="pos_aff father", y="pos_aff mother")
```


**Contemporaneous dyad plot**: the plot is composed of scatterplots for the 'neg_aff' variables, with the father's neg_aff on the x-axis and the mother's neg_aff on the y-axis. 
It helps investigating the contemporaneous relationship between the father and the mother within dyads.

```{r, fig.height=8, fig.width=8}
ggplot(data = data_dyad, aes(x = neg_aff_2, y = neg_aff_1)) +
  geom_jitter() +
  facet_wrap(dyad~., ncol=3) +
  labs(title="Contemporaneous dyad relationship in neg_aff", x="neg_aff father", y="neg_aff mother")
```
...
participants_book(data, 
        ...
        timevar="start", 
        validvar="valid", 
        compliancevar = "compliance",
        viz=list(c("ts","hist"), c("ts","hist")),
        html_output="report_examples/tables/Participant_book_example.html")
```
