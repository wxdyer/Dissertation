...
# Part 6 completer IDs (any participant with complete Part 6 data)
...
This section characterizes how many surveys participants submitted, regardless of item completeness. A submitted survey is defined as any observation with a non-missing `completion_time`. This provides a broad view of participant engagement with the EMA protocol.

```{r submission-summary}
# Overall submission rate — non-missing completion_time
submission_df <- study_sample %>%
  group_by(participant_id) %>%
  summarise(
    submitted_beeps  = sum(!is.na(completion_time)),
    max_dayno        = max(dayno, na.rm = TRUE),
    total_possible   = if_else(max_dayno <= 3, 18L, 60L),
    completion_group = if_else(max_dayno <= 3,
                                "Part 2 Only (3 days)",
                                "Parts 2 + 6 (10 days)"),
    submission_rate  = submitted_beeps / total_possible
  ) %>%
  ungroup()
...
submission_df %>%
  ...
  summarise(
    n                    = n(),
    mean_submitted       = round(mean(submitted_beeps), 1),
    mean_possible        = round(mean(total_possible), 1),
    mean_submission_rate = round(mean(submission_rate), 3),
    min_submission_rate  = round(min(submission_rate), 3),
    max_submission_rate  = round(max(submission_rate), 3)
  ) %>%
  kable(caption = "Overall survey submission rate by study completion group (submitted = non-missing completion_time, regardless of item completeness)") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"),
                full_width = FALSE)
```

```{r submission-plot}
ggplot(submission_df, aes(x = submission_rate, fill = completion_group)) +
  ...
  labs(
    title = "Distribution of overall survey submission rate by study completion group",
    subtitle = "Submission = any survey with non-missing completion_time",
    x     = "Submission rate (% of possible EMA prompts)",
    y     = "Number of participants"
  ) +
  theme_minimal() +
...
## Valid Observations Rate

This section characterizes the proportion of submitted surveys that meet all item-completeness criteria specific to this project (non-missing mood, smoke, before_survey, and at least one branch variable -- pleasant / unpleasant). These are the observations available for within-person analyses.
...
compliance_df %>%
  ...
  kable(caption = "Valid observations summary by study completion group (valid = item-complete per project-specific criteria)") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"),
                full_width = FALSE)
```
...
ggplot(compliance_df, aes(x = compliance, fill = completion_group)) +
  ...
  facet_wrap(~ completion_group, ncol = 1) +
  labs(
    title = "Distribution of valid observation rate by study completion group",
    subtitle = "Valid = item-complete per project-specific criteria",
    x     = "Valid observation rate (% of possible EMA prompts)",
    y     = "Number of participants"
  ) +
  theme_minimal() +
  theme(legend.position = "none")
...
## Valid Observations: Associations with Participant Characteristics

`r committee_note("The committee asked whether there are significant trends in non-compliance that should be accounted for (e.g., participant characteristics). The analysis below examines associations between baseline characteristics and the rate of item-complete (valid) observations.")`
...
as.data.frame(display_mat) %>%
  kable(caption = "Correlations between valid observation rate and baseline characteristics. * p < .05, ** p < .01, *** p < .001, . p < .10") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"),
                full_width = FALSE)
```
...
`r section_summary("Overall survey submission rates were generally high across participants. Valid observation rates — reflecting item completeness per project-specific criteria — were also examined and are the primary metric for available cases in dissertation analyses. A participant-level validity audit identified cases where the branch variable (pleasant/unpleasant) was the limiting factor for observation-level validity — notably, participant 223 had zero valid observations during Part 6 and only 2 during Part 2, rendering them unusable for analyses requiring the full validity criteria. Missingness was concentrated in the branch variables, consistent with the branching structure of the survey. Beep timing followed the expected 2-hour interval sampling scheme across weekdays and weekends.")`
...
engagement_labels <- c("Valid Obs Rate", "Dropout\n(1=Did not complete Part 6)", "N Valid Obs")
...
all_outcome_labels <- c("Valid Obs Rate", "Dropout (1=Did not complete Part 6)",
...
**Operationalization of abstinence:** A participant is considered abstinent on a given Part 6 day if `smoked_since_last_survey == 0` across all valid beeps on that day. Consecutive abstinence days are counted as unbroken streaks — if a participant smokes on any day, the streak resets to zero. This definition differs from the "Quitter" category in the Change over Time section, which is defined as a mean Part 6 daily smoking value ≤ 0.1 across all Part 6 days. As a result, the N of participants appearing in the abstinence streak table on Day 1 may differ from the N of Quitters in the change category table — a participant can qualify as a Quitter based on their overall average while still having smoked briefly early in Part 6, and vice versa.
...
# Change categories are defined using mean Part 6 daily smoking relative to
# the individual Part 2 baseline mean:
#   Quitter   = mean Part 6 daily smoking <= 0.1 (effectively zero across Part 6)
#   Reducer   = mean Part 6 daily smoking < 80% of Part 2 baseline mean
#   Stable    = mean Part 6 daily smoking within 20% of Part 2 baseline mean
#   Increaser = mean Part 6 daily smoking > 120% of Part 2 baseline mean
#
# Note: This operationalization differs from the consecutive abstinence streak
# used in the abstinence days section. A Quitter here may have smoked briefly
# early in Part 6 but averaged near-zero across the full period.
# Conversely, a participant with a perfect streak on early days might average
# above zero overall due to later smoking. This explains any N discrepancy
...
# --- Plot 2a: Mood (thick) + Pleasant (dotted) by change category ---
group_means_mood <- daily_aim3 %>%
  group_by(dayno, change_cat) %>%
  summarise(
    group_mood    = mean(mean_mood, na.rm = TRUE),
    group_pleasant = mean(mean_pleasant, na.rm = TRUE),
    .groups = "drop"
  )
...
ggplot(daily_aim3, aes(x = dayno)) +
  geom_line(aes(y = mean_mood, group = participant_id),
            alpha = 0.15, color = "gray60") +
  geom_line(data = group_means_mood,
            aes(y = group_mood, color = change_cat),
            linewidth = 1.5) +
  geom_line(data = group_means_mood,
            aes(y = group_pleasant, color = change_cat),
  ...
  annotate("text", x = 7, y = Inf, label = "Part 6\n(Incentive)",
           vjust = 1.5, size = 3, color = "gray40") +
  labs(
    title    = "Mood and pleasant rating trajectories by change category (Part 6 completers)",
    subtitle = "Solid lines = mood; dotted lines = pleasant rating; bold = group means; light gray = individual trajectories",
    x        = "Study day",
    y        = "Mean daily value",
    color    = "Change category"
  ) +
...
# --- Plot 2b: Mood (thick) + Unpleasant (dotted) by change category ---
group_means_mood2 <- daily_aim3 %>%
  group_by(dayno, change_cat) %>%
  summarise(
    group_mood      = mean(mean_mood, na.rm = TRUE),
    group_unpleasant = mean(mean_mood - mean_mood + mean_pleasant, na.rm = TRUE),
    .groups = "drop"
  )

# Recompute with unpleasant properly
group_means_unpleasant <- smoking_completers_aim3 %>%
  left_join(aim3_data %>% select(participant_id, change_cat),
            by = "participant_id") %>%
  filter(!is.na(change_cat)) %>%
  group_by(participant_id, dayno) %>%
  summarise(
    mean_mood      = mean(mean_mood, na.rm = TRUE),
    .groups = "drop"
  ) %>%
  left_join(
    study_sample %>%
      ...
      filter(valid == 1, participant_id %in% valid_completers) %>%
      group_by(participant_id, dayno) %>%
      summarise(mean_unpleasant = mean(unpleasant, na.rm = TRUE), .groups = "drop"),
    by = c("participant_id", "dayno")
  ) %>%
  left_join(aim3_data %>% select(participant_id, change_cat), by = "participant_id") %>%
  filter(!is.na(change_cat)) %>%
  group_by(dayno, change_cat) %>%
  summarise(
    group_mood       = mean(mean_mood, na.rm = TRUE),
    group_unpleasant = mean(mean_unpleasant, na.rm = TRUE),
    .groups = "drop"
  )
...
ggplot(daily_aim3, aes(x = dayno)) +
  geom_line(aes(y = mean_mood, group = participant_id),
            alpha = 0.15, color = "gray60") +
  geom_line(data = group_means_unpleasant,
            aes(y = group_mood, color = change_cat),
            linewidth = 1.5) +
  geom_line(data = group_means_unpleasant,
            aes(y = group_unpleasant, color = change_cat),
            linewidth = 1, linetype = "dotted") +
  geom_vline(xintercept = 3.5, linetype = "dashed", color = "gray40") +
  scale_color_manual(values = change_colors) +
  scale_x_continuous(breaks = 1:10) +
  ...
  annotate("text", x = 7, y = Inf, label = "Part 6\n(Incentive)",
           vjust = 1.5, size = 3, color = "gray40") +
  labs(
    title    = "Mood and unpleasant rating trajectories by change category (Part 6 completers)",
    subtitle = "Solid lines = mood; dotted lines = unpleasant rating; bold = group means; light gray = individual trajectories",
    x        = "Study day",
    y        = "Mean daily value",
    color    = "Change category"
  ) +
...
The table below summarizes available participants and valid observations for each dissertation Aim. Full participant-level detail is provided in the Validity Audit section above, and the CONSORT diagram in Section 3 documents the flow from screening through study completion.

```{r available-cases}
# Aims 1 & 2 — Part 2 data
n_part_aim12 <- study_sample %>%
  filter(study_part == 2) %>%
  mutate(valid = check_validity(.)) %>%
  filter(valid == 1) %>%
  distinct(participant_id) %>%
...
n_obs_aim12 <- study_sample %>%
  filter(study_part == 2) %>%
  mutate(valid = check_validity(.)) %>%
  filter(valid == 1) %>%
  nrow()

# Aim 3 — Part 6 data (valid completers only)
n_part_aim3 <- study_sample %>%
  filter(study_part == 6) %>%
  mutate(valid = check_validity(.)) %>%
  filter(valid == 1) %>%
  distinct(participant_id) %>%
...
tibble(
  Aim              = c("Aims 1 & 2 (Part 2 data)",
                       "Aim 3 (Part 6 data)"),
  N_participants   = c(n_part_aim12, n_part_aim3),
  N_valid_obs      = c(n_obs_aim12, n_obs_aim3),
  Notes            = c(
    "Full Part 2 analytic sample; excludes participant 416 (missing baseline covariates) from covariate-dependent models",
    "Part 6 completers with valid observations; excludes participant 223 (zero valid Part 6 observations)"
  )
) %>%
  kable(caption = "Available cases by dissertation Aim") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"),
                full_width = TRUE)
```
---
