...
```{r consort}
# Participant counts for CONSORT diagram
...
n_total <- study_sample %>%
  distinct(participant_id) %>%
  nrow()

n_part2_only <- n_total - n_part6

cat("Participants with Part 2 data:", n_part2, "\n")
cat("Participants with Part 6 data:", n_part6, "\n")
cat("Participants with Part 2 data only:", n_part2_only, "\n")
cat("Dropout between Part 2 and Part 6:", n_part2 - n_part6, "\n")
```
...
```{r available-cases}
# Part 2 participants (Aims 1 and 2)
n_aim1_2 <- study_sample %>%
  filter(study_part == 2) %>%
  mutate(valid = check_validity(.)) %>%
  filter(valid == 1) %>%
  distinct(participant_id) %>%
...
tibble(
  Aim            = c("Aims 1 & 2 (Part 2)", "Aim 3 (Part 6)"),
  N_participants = c(n_aim1_2, n_aim3),
  N_observations = c(n_obs_aim1_2, n_obs_aim3)
) %>%
  kable(caption = "Available cases by Aim") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"),
                full_width = FALSE)
```

*See Section 3 for the CONSORT diagram documenting participant flow.*
