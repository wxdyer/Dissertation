...
`r txt('esm-mod','Modification',"In order to maintain these data but in a format compatible with numeric formatting, a variable to track opted out instances is created to check whether further actions to account for these cases must be taken.")` 
...
`r txt('esm-mod','Modification',"The chunk below formats time variables as numeric for later POSIXct")`
...
`r txt(id='esm-issue',title='Issue',text="Baseline data are raw. Some measures of time-invariant characteristics (e.g., Fagerstrom Test of Cigarette Dependence) need scoring and some variables require renaming/relabeling.")` 
...
`r txt(id='esm-issue',title='Issue',text="he EMA data alone do not contain all participant data needed for analyses. They must be combined with baseline data.")`
...
## Duplication
Participant #102 had two observations within a single observation 2-hr observation window due to a time-out issue. The time-out observation contained no data and was removed.
...
`r txt(id='esm-issue',title='Issue',text=" The above table reveals that participant 102 submitted 61 total observations, which does not make logical sense because the maximum observations a participant can submit for our study is 60. This issue arose when one of their surveys was interrupted and resent to the participant, but both the interrupted and the actual observation were saved. This interrupted observation contained no usable data.")`
...
study_sample %>%
  mutate(
    # Identify observations where both branch variables are missing
    both_missing = is.na(pleasant) & is.na(unpleasant),
    # Identify observations where branch variables are incorrectly displayed
  ...
  summarise(
    ...
    n_pleasant_display_error = sum(pleasant_display_error),
    n_unpleasant_display_error = sum(unpleasant_display_error)
  )
```
...
`r txt('esm-mod','Modification',"Below, we create 'dayno' (day since first beep was sent), 'duration' representing the total length of one's participation, and 'beepno' which is the observation number within a given day.")`
```{r}
study_sample <- study_sample %>%
