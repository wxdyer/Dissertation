...
```
```{r}
table(study_sample_full$before_survey, useNA="ifany")
table(study_sample_part2_only$before_survey, useNA="ifany")
```
...
```{r}
sum(duplicated(study_sample_part2_only))
```
```{r}
sum(duplicated(study_sample_part2_only))
```
...
```{r}
study_sample_part2_only[!is.na(study_sample_part2_only$init_time),] %>% # Select answered observations
...
dup_answers <- study_sample_part2_only %>%
...
```{r}
gg_miss_upset(study_sample_part2_only)
```
...
# Reorder variables
data = study_sample_part2_only %>% arrange(participant_id, completion_time)

# Overiew of missing values
vis_miss(study_sample_part2_only)
```
...
study_sample_part2_only %>%
  ...
  ggplot(aes(x = factor(participant_id), y = n_completed_beeps)) +
    ...
    scale_y_continuous(breaks = seq(0, max(study_sample_part2_only$obsno, na.rm = TRUE), 5)) +
    labs(
...
df <- study_sample_part2_only[, c("participant_id", "obsno",
...
```{r}
df_interact <- study_sample_part2_only %>%
  mutate(
    hour = hour(init_time),
...
study_sample_part2_only_intervals <- study_sample_part2_only %>%
  filter(valid == 1) %>% 
...
study_sample_part2_only_intervals %>%
...
```{r}
obsno_max = 18
study_sample_part2_only$compliance = ave(study_sample_part2_only$valid, study_sample_part2_only$participant_id, FUN=function(x) sum(x, na.rm=TRUE)) / obsno_max
...
```
```{r}
# Save the study_sample_part2_only dataframe as an RDS file
saveRDS(study_sample_part2_only, file = here("output", "cleaned_study_sample_part2_only.RDS"))
```
