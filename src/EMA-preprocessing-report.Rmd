...
This section is dedicated to the first look at the data, the merging of data sources, the first basic preprocessing methods (e.g., duplicates, branching items check), and checking the variable consistency when the data have just been imported.
...
analytic_sample <- c(102, 104, 107, 112, 113, 115, 116, 124, 125, 126, 127, 128, 133, 141, 150, 151, 154, 159, 163, 164, 166, 169, 175, 177, 180, 186, 188, 192, 195, 196, 197, 198, 199, 202, 206, 209, 210, 213, 214, 216, 218, 221, 222, 223, 233, 238, 240, 246, 252, 257, 262, 268, 273, 274, 278, 281, 282, 286, 287, 290, 291, 295, 297, 300, 301, 304, 310, 314, 320, 321, 329, 332, 340, 341, 342, 344, 346, 349, 351, 356, 362, 363, 369, 370, 372, 381, 383, 393, 394, 399, 416, 423, 427, 428, 430)
...
baseline_clean <- baseline %>%
  select(
    ...
    panas_pos,
    panas_neg,
    q23, q24, q25, #age of first cig puff, age of first full cig, age when smoking regularly began
    q80, #past 30-day marijuana use
    q71, #past 30-day e-cig use
  )
baseline_clean <- baseline_clean %>%
  mutate(employment_status = recode(as.character(employment_status),
    "1" = "Part-time",
    "2" = "Full-time",
...
# Create 'obsno' variable, representing the serial order of observations within-participant, and 'dayno' variable, based on the date of the first completed beep (dayno == 1). This is to prepare for biochemical abstinence verification data merging.
study_sample <- study_sample %>%
  arrange(participant_id, completion_time) %>%
  group_by(participant_id) %>%
  mutate(
    obsno = row_number(),
    dayno = dense_rank(as.Date(completion_time))
  ) %>%
  ungroup()
```
...
Baseline and EMA data are now merged.

`r txt(id='esm-issue',title='Issue',text="Daily biochemical abstinence verification (i.e., breath Carbon Monoxide (CO) concentration) data must be included for validity checks.",FALSE)`

`r txt('esm-mod','Modification',"Biochemical abstinence verification data are read in, cleaned, and merged with the EMA+baseline dataset.")`

`r button(text = "Description")`

```{r}
# IDs for analytic sample (Part 2 subset)
analytic_sample <- c(102, 104, 107, 112, 113, 115, 116, 124, 125, 126, 127, 128, 133, 141,
                     150, 151, 154, 159, 163, 164, 166, 169, 175, 177, 180, 186, 188, 192,
                     195, 196, 197, 198, 199, 202, 206, 209, 210, 213, 214, 216, 218, 221,
                     222, 223, 233, 238, 240, 246, 252, 257, 262, 268, 273, 274, 278, 281,
                     282, 286, 287, 290, 291, 295, 297, 300, 301, 304, 310, 314, 320, 321,
...
co_path <- here("data/DVAL_CO-video_data_full.xlsx")

# ---------------------------
# Function to clean and pivot CO sheets (robust co_vid_time)
# ---------------------------
clean_co_sheet <- function(sheet_name, day_offset = 0, subset_ids = NULL) {
  
  raw <- read_excel(co_path, sheet = sheet_name,
                    na = c("N/A", "n/a")) %>%
    clean_names() %>%
    select(-notes, -verified)
  
  if(!is.null(subset_ids)) {
    raw <- raw %>% filter(participant_id %in% subset_ids)
  }
  
  # Identify columns
  co_level_cols <- grep("_co_level$", names(raw), value = TRUE)
  time_cols <- grep("_time_recorded$", names(raw), value = TRUE)
  
  raw <- raw %>%
    mutate(across(all_of(c(co_level_cols, time_cols)), as.character)) %>%
    mutate(across(all_of(time_cols), ~ trimws(.)))
  
  long <- raw %>%
    pivot_longer(
      cols = -participant_id,
      names_to = c("co_day", ".value"),
      names_pattern = "day_(\\d+)_(.+)"
    ) %>%
    mutate(co_day = as.integer(co_day) + day_offset) %>%
    rename(
      co_date = date,
      co_time_recorded = time_recorded,
      co_level = co_level,
      co_state_smoked = state_smoked,
      co_correct_procedure = correct_procedure
    ) %>%
    mutate(
      co_level = suppressWarnings(as.numeric(co_level)),
      co_date = as.Date(co_date),
      
      # ---- Robust parsing for co_time_recorded ----
      # Remove Excel dummy date if present
      co_time_only = sub("1899-12-31\\s+", "", co_time_recorded),
      # Combine co_date + time to POSIXct
      co_vid_time = as.POSIXct(
        paste(co_date, co_time_only),
        format = "%Y-%m-%d %H:%M:%S",
        tz = "America/New_York"
      ),
      
      co_state_smoked = case_when(
        tolower(co_state_smoked) %in% c("yes","y","true") ~ TRUE,
        tolower(co_state_smoked) %in% c("no","n","false") ~ FALSE,
        TRUE ~ NA
      ),
      co_correct_procedure = case_when(
        tolower(co_correct_procedure) %in% c("yes","y","true") ~ TRUE,
        tolower(co_correct_procedure) %in% c("no","n","false") ~ FALSE,
        TRUE ~ NA
      )
    ) %>%
    select(participant_id, co_day, co_date, co_time_recorded,
           co_level, co_state_smoked, co_correct_procedure, co_vid_time)
  
  return(long)
}

# ---------------------------
# Process Part 2 (days 1–3) for analytic sample
# ---------------------------
part2_long <- clean_co_sheet(
  sheet_name = "Video info - Part 2 (3-day)",
  day_offset = 0,
  subset_ids = analytic_sample
)

# ---------------------------
# Process Part 6 (days 4–10)
# ---------------------------
part6_long <- clean_co_sheet(
  sheet_name = "Video info - Part 6 (7-day)",
  day_offset = 3
)

# ---------------------------
# Combine Part 2 + Part 6
# ---------------------------
co_long <- bind_rows(part2_long, part6_long)

# ---------------------------
# Expand full co_day 1–10 for all participants
# ---------------------------
participants <- unique(study_sample$participant_id)

co_data <- expand.grid(
  participant_id = participants,
  co_day = 1:10
) %>%
  left_join(co_long, by = c("participant_id", "co_day")) %>%
  arrange(participant_id, co_day)

# ---------------------------
# Merge CO data with EMA dataset
# ---------------------------

study_sample <- study_sample %>%
  left_join(co_data, by = c("participant_id", "dayno" = "co_day"))

# Optional: remove intermediate objects
rm(part2_long, part6_long, co_long, co_data)

```

`r endbutton()` 

We now have a single dataset containing EMA, baseline, and CO video data, called study_sample.
`r txt('esm-inspect','Inspection',"study_sample dataset meta-info:",FALSE)` 
...
study_sample %>%
  group_by(participant_id) %>%
  summarise(n_employment_values = n_distinct(employment_status)) %>%
  filter(n_employment_values > 1)
...
study_sample %>%
  group_by(participant_id) %>%
  summarise(n_q23_values = n_distinct(q23)) %>% # Q23: Age at first cigarette (even a puff)
  filter(n_q23_values > 1)

study_sample %>%
  group_by(participant_id) %>%
  summarise(n_q24_values = n_distinct(q24)) %>% # Q24: Age at first full cigarette
  filter(n_q24_values > 1)

study_sample %>%
  group_by(participant_id) %>%
  summarise(n_q25_values = n_distinct(q25)) %>% # Q25: Age first started smoking regularly
  filter(n_q25_values > 1)

study_sample %>%
  group_by(participant_id) %>%
  summarise(n_q71_values = n_distinct(q71)) %>% # Q71: E-cigarette use frequency (past 30 days)
  filter(n_q71_values > 1)

study_sample %>%
  group_by(participant_id) %>%
  summarise(n_q80_values = n_distinct(q80)) %>% # Q80: Cannabis use frequency (past 30 days)
  filter(n_q80_values > 1)
```
Note: Participant 416 is missing all baseline variables due to absence from the baseline questionnaire files. This is participant-level missingness and is noted — their EMA observations are retained in the dataset but baseline covariates will be unavailable for this participant in analyses requiring them.
...
Our analyses require the creation of time-related variables: 'obsno', which represents the serial order in which a given observation occurs (created above), 'dayno', which represents the day since the participant's first beep (created above), and 'beepno', which represents the beep number (1-6) within a day for a participant (created in next code chunk), and 'duration', which is a time-invariant measure of the total length of one's participation (created in next code chunk). 

`r txt('esm-mod','Modification',"We created the 'obsno' and 'dayno' variables earlier. Here, we create 'dayno' and 'duration'.")` 

`r button(text = "Description")` 
```{r}
# Step 1: time components and duration
study_sample <- study_sample %>%
  arrange(participant_id, completion_time) %>%
  group_by(participant_id) %>%
  mutate(
...
# Step 2: beepno — must run after dayno is correctly defined
study_sample <- study_sample %>%
  arrange(participant_id, dayno, completion_time) %>%
  group_by(participant_id, dayno) %>%
  mutate(beepno = ifelse(is.na(dayno), NA_integer_, row_number())) %>%
  ungroup()
```
