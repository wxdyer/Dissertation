...
# Create a new comprised of participants who completed at least Part 2 of the parent study.
...
# Subset the sample to only include participants who completed at least part 2 of the parent study
study_sample <- study_sample %>%
  filter(participant_id %in% study_sample_ids) %>%
  arrange(participant_id, completion_time) %>%
  group_by(participant_id) %>%
  mutate(obsno = row_number()) %>%   # serial observation number within person
  ungroup() %>%
  select(participant_id, everything())
```

study_sample dataset meta-info:
```{r}
...
comparison_overall <- tibble(
  ...
  Valid_Observations = c(
    ...
    sum(study_sample$valid == 1, na.rm = TRUE)
  )
) %>%
  mutate(
...
comparison_overall
```
The full sample contains more available cases for analysis than the Part 2 only sample, despite attrition. The remaining data preprocessing and characteristics reporting will reflect actions performed on the full dataset (study_sample).
