...
# Create obsno variable representing the serial order of observations within-participant, to prepare for biochemical abstinence verification data.
...
part6_long <- part6_raw %>%
  pivot_longer(
    cols = matches("^day_\\d+_"),
    names_to = c("day_temp", "measure"),
    names_pattern = "day_(\\d+)_(.+)",
    values_drop_na = FALSE
  ) %>%
  mutate(
    co_day = as.integer(day_temp) + 3
  ) %>%
  select(participant_id, co_day, measure, value) %>%
  pivot_wider(
    names_from = measure,
    values_from = value
  ) %>%
...
# Convert types and combine date + time
part6_long <- part6_long %>%
  mutate(
    co_date = as.Date(co_date),
    co_time_recorded = parse_date_time(co_time_recorded, orders = "I:M p"),
    co_level = as.numeric(co_level),
    co_state_smoked = ifelse(tolower(co_state_smoked) %in% c("yes", "y", "true"), TRUE,
                             ifelse(tolower(co_state_smoked) %in% c("no", "n", "false"), FALSE, NA)),
    co_correct_procedure = ifelse(tolower(co_correct_procedure) %in% c("yes", "y", "true"), TRUE,
                                  ifelse(tolower(co_correct_procedure) %in% c("no", "n", "false"), FALSE, NA)),
    co_vid_time = as.POSIXct(
      paste(co_date, format(co_time_recorded, "%I:%M %p")),
      format = "%Y-%m-%d %I:%M %p",
      tz = "America/New_York"
    )
  ) %>%
