...
```{r}
# Import Part 6 CO data sheet
co_path <- here("data/DVAL_CO-video_data_full.xlsx")
sheet_name <- "Video info - Part 6 (7-day)"

part6_raw <- read_excel(co_path, sheet = sheet_name,
                        na = c("N/A", "n/a")) %>%
  clean_names() %>%
  select(-notes, -verified)

# Temporarily coerce mixed-type columns to character and trim whitespace
co_level_cols <- grep("_co_level$", names(part6_raw), value = TRUE)
time_cols <- grep("_time_recorded$", names(part6_raw), value = TRUE)
...
# Pivot wide -> long
part6_long <- part6_raw %>%
  pivot_longer(
    cols = -participant_id,
    names_to = c("co_day", ".value"),
    names_pattern = "day_(\\d+)_(.+)"
  ) %>%
  mutate(co_day = as.integer(co_day) + 3) %>%
  rename(
    co_date = date,
...
# Convert types and combine date + time (suppress warnings introduced when raw missing cases represented by the literal text "N/A" get converted to actual missing cases)
part6_long <- part6_long %>%
  mutate(
    co_level = suppressWarnings(as.numeric(co_level)),
    co_time_parsed = suppressWarnings(
      parse_date_time(co_time_recorded, orders = "I:M p", tz = "UTC")
    ),
    co_date = as.Date(co_date),
    co_vid_time = as.POSIXct(
      paste(co_date, format(co_time_parsed, "%H:%M:%S")),
      format = "%Y-%m-%d %H:%M:%S",
      tz = "America/New_York"
    ),
...
# Expand full co_day 4–10 for all participants
...
co_data <- expand.grid(
  ...
  left_join(part6_long, by = c("participant_id", "co_day")) %>%
...
rm(part6_raw, part6_long, co_data)
```
