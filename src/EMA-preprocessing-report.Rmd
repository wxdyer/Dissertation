...
Part 2 EMA raw dataset meta-info:
```{r}
dataInfo(file_path = part2_path, 
         ...
         timevar = "init_time")
```
Part 6 EMA raw dataset meta-info:
```{r}
...
## Modification text: data are subsetted to include only participants who completed Part 2 (Part 6 EMA is already subsetted to include only-completers of Part 6).
```{r}
...
# Inspect opting out behavior by participant
```{r}
#mood
ema_master %>%
  select(
...
```
Note: Participant #372 opted out of responding to 'mood', 'before_survey' and 'smoke' on one occasion; Participant #427 opted out of responding to 'mood' once. Because these behavioral data are not useful moving forward, these data will be treated as missing. Now that this check is complete, all created [variable]_optout variables are deleted in the following step.

# Delete optout variables.
```{r}
ema_master <- ema_master %>%
...
# Create two new samples: one comprised of participants who completed Part 2 data (3 days, N = 94) and a "full" EMA dataset comprised of participants who completed both Parts 2 and 6 (10 days, N = 59)

```{r}
# IDs for Part 2 only sample
study_part2_ids <- c(
  102, 104, 107, 112, 113, 115, 116, 124, 125, 126, 127, 128, 133, 141, 
  150, 151, 154, 159, 163, 164, 166, 169, 175, 177, 180, 186, 188, 192, 
  195, 196, 197, 198, 199, 202, 206, 209, 210, 213, 214, 216, 218, 221, 
...
# IDs for full sample (Completers of Part 2 and Part 6 )
study_full_ids <- c(
  102, 104, 107, 112, 116, 124, 125, 126, 128, 141,
...
# Subset ema_master for Part 2 only sample
study_sample_part2_only <- study_sample %>%
  filter(participant_id %in% study_part2_ids) %>%
  group_by(participant_id) %>%
  arrange(completion_time, .by_group = TRUE) %>%
  mutate(
    first_date = as.Date(min(completion_time, na.rm = TRUE)),
    day_number = as.integer(as.Date(completion_time) - first_date) + 1
  ) %>%
  filter(day_number <= 3) %>%
  ungroup() %>%
  select(participant_id, everything())

#Create 'obsno'  variable to represent serial order of an observation in study_sample_part2_only
study_sample_part2_only <- study_sample_part2_only %>%
...
# Subset ema_master for full sample
study_sample_full <- study_sample %>%
  filter(participant_id %in% study_full_ids) %>%
  select(participant_id, everything())  # move participant_id to first column

#Create 'obsno'  variable to represent serial order of an observation in study_sample_full
study_sample_full <- study_sample_full %>%
  arrange(participant_id, completion_time) %>%
  group_by(participant_id) %>%
  mutate(obsno = row_number()) %>%  # continuous numbering across part 2 + 6
  ungroup() %>%
  select(participant_id, everything())
```

study_sample_part2_only dataset meta-info:
```{r}
cat("## Path      : in-memory dataframe\n")
cat("## Extension : RDS (original format, now in-memory)\n")
cat("## Size      :", format(object.size(study_sample_part2_only), units = "auto"), "\n\n")

# Dimensions
cat("## ncol      :", ncol(study_sample_part2_only), "\n")
cat("## nrow      :", nrow(study_sample_part2_only), "\n\n")

# Number of participants
id_var <- "participant_id"
n_participants <- n_distinct(study_sample_part2_only[[id_var]])
cat("## Number of participants :", n_participants, "\n")

# Average number of observations per participant
avg_obs <- nrow(study_sample_part2_only) / n_participants
cat("## Average number of observations per participant :", round(avg_obs, 1), "\n\n")

# Time period
time_var <- "init_time"
if(time_var %in% names(study_sample_part2_only)) {
  time_min <- min(study_sample_part2_only[[time_var]], na.rm = TRUE)
  time_max <- max(study_sample_part2_only[[time_var]], na.rm = TRUE)
...
# Variable names
cat("## Variables :", paste(names(study_sample_part2_only), collapse = ", "), "\n")
```

study_sample_full dataset meta-info:
```{r}
cat("## Path      : in-memory dataframe\n")
cat("## Extension : RDS (original format, now in-memory)\n")
cat("## Size      :", format(object.size(study_sample_full), units = "auto"), "\n\n")

# Dimensions
cat("## ncol      :", ncol(study_sample_full), "\n")
cat("## nrow      :", nrow(study_sample_full), "\n\n")

# Number of participants
id_var <- "participant_id"
n_participants <- n_distinct(study_sample_full[[id_var]])
cat("## Number of participants :", n_participants, "\n")

# Average number of observations per participant
avg_obs <- nrow(study_sample_full) / n_participants
cat("## Average number of observations per participant :", round(avg_obs, 1), "\n\n")

# Time period
time_var <- "init_time"
if(time_var %in% names(study_sample_full)) {
  time_min <- min(study_sample_full[[time_var]], na.rm = TRUE)
  time_max <- max(study_sample_full[[time_var]], na.rm = TRUE)
  cat("## Period : from", format(time_min, "%Y-%m-%d %H:%M:%S"),
      "to", format(time_max, "%Y-%m-%d %H:%M:%S"), "\n\n")
} else {
  cat("## Period : init_time variable not found in dataset\n\n")
...
# Variable names
cat("## Variables :", paste(names(study_sample_full), collapse = ", "), "\n")
```
#Examine number of observations sent to participants
```{r}
obsno_summary <- study_sample_full %>%
  group_by(participant_id) %>%
  summarise(
...
```
Issue text: Participant 102 submitted 61 total observations, which does not make logical sense because the maximum observations for our study is 60. This issue arose when one of the participant's surveys was interrupted. This interrupted observation contained no usable data, and thus is removed in the step below. 

#Remove interrupted observation from participant 102 and review obsno summary. 
```{r}
study_sample_full <- study_sample_full %>%
...
# Apply validity function to both datasets to see which has more valid observations
study_sample_part2_only$valid <- check_validity(study_sample_part2_only)
study_sample_full$valid       <- check_validity(study_sample_full)

# Compare between datasets
comparison_overall <- tibble(
  Dataset = c("Part 2 Only", "Full Sample"),
  Total_Observations = c(
    nrow(study_sample_part2_only),
    nrow(study_sample_full)
  ),
  Valid_Observations = c(
    sum(study_sample_part2_only$valid == 1, na.rm = TRUE),
    sum(study_sample_full$valid == 1, na.rm = TRUE)
  )
) %>%
  mutate(
    Percent_Valid = round(100 * Valid_Observations / Total_Observations, 2)
  )

comparison_overall
```
The full sample contains more available cases for analysis than the Part 2 only sample, despite attrition. The remaining data preprocessing and characteristics reporting will reflect actions performed on the full dataset (study_sample_full).

## First glimpse
The following chunk helps to have a first insight on what the dataset looks like.

```{r}
dim(study_sample_full)
```
```{r}
head(study_sample_full)
```

```{r}
tail(study_sample_full)
```
```{r}
glimpse(study_sample_full)
```
```{r}
summary(study_sample_full)
```
...
```{r}
sum(duplicated(study_sample_full))
```
```{r}
sum(duplicated(study_sample_full))
```
...
```{r}
study_sample_full[!is.na(study_sample_full$init_time),] %>% # Select answered observations
...
branch_check <- study_sample_part2_only %>%
  mutate(
...
```
```{r}
#Check for branch violations
branch_violations <- study_sample_part2_only %>%
  summarise(
    pleasant_when_not_selected =
      sum(!is.na(pleasant) & before_survey != "It was generally pleasant"),
    unpleasant_when_not_selected =
      sum(!is.na(unpleasant) & before_survey != "It was generally unpleasant"),
    both_present =
      sum(!is.na(pleasant) & !is.na(unpleasant))
  )

branch_violations
```
Branching integrity was confirmed, as all observations with both pleasant and unpleasant missing corresponded exactly to neutral or missing before_survey responses, and no cases were identified in which a branch variable was displayed when its corresponding condition was not selected or in which both branch variables were simultaneously present.

# We can get a sense for patterns of missingness by identifying the potential relationships or dependencies in missing values between variables (bottom) and the occurrences of each pattern (above).
```{r}
gg_miss_upset(study_sample_full)
```
...
data <- study_sample_full %>% 
  arrange(participant_id, completion_time)

vis_miss(study_sample_full) +
  theme(
...
study_sample_full %>%
  ...
  ggplot(aes(x = factor(participant_id), y = n_completed_beeps)) +
    ...
    scale_y_continuous(breaks = seq(0, max(study_sample_full$obsno, na.rm = TRUE), 5)) +
    labs(
...
df <- study_sample_full[, c("participant_id", "obsno",
...
study_sample_full_intervals <- study_sample_full %>%
  filter(valid == 1) %>% 
...
```
## Compliance Rate
We computed participant compliance as the proportion of valid EMA observations out of 60 possible prompts per participant, based on the valid observation definition described above.
```{r}
study_sample_full %>%
  group_by(participant_id) %>%
  slice(1) %>%
  ...
  labs(
    ...
    x = "Compliance (% of 60 possible observations)",
...
```
```{r}
# Save the study_sample_full dataframe as an RDS file
saveRDS(study_sample_full, file = here("output", "cleaned_study_sample_full.RDS"))
```
